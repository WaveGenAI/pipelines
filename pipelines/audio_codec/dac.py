...
class DAC(PreTrainedModel):
    ...
    def load_audio(self, audio: str | numpy.ndarray | torch.Tensor):
        signal = AudioSignal(audio, sample_rate=44100)
        waveform = signal.audio_data.squeeze(0)  # (n_channels, length)
        return waveform, signal.sample_rate
    ...
    def encode(self, audio_fname: str):
        ...
        waveform = self.resample_audio(
            waveform, orig_sr=sr, target_sr=self.model_type_by_sampling_freq_int
        )
