...
parser.add_argument("--directory", help="output directory path", required=True)
...
def generator_urls():
    ...
    with open(args.input, "r", encoding="utf-8") as f:
        for line in f.readlines():
            ...
            while os.path.exists(os.path.join(args.directory, file_idx + ".mp3")):
                file_idx = str(random.randint(0, 10**10)).zfill(10)

            yield urls, metatags, file_idx

            # write a json file with the metadata
            json_path = os.path.join(args.directory, file_idx + ".json")
            data = {"urls": urls, "metatags": metatags}
            with open(json_path, "w", encoding="utf-8") as f:
                json.dump(data, f)


downloader.download_all(generator_urls(), args.directory)
